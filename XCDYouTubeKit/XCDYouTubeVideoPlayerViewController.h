 ...
 *
 *  Ideally, there should be a `MPMoviePlayerPlaybackDidFinishErrorUserInfoKey` declared near to
 * `MPMoviePlayerPlaybackDidFinishReasonUserInfoKey` in MPMoviePlayerController.h but since it doesn't exist, here is a
 * convenient constant key.
 */
MP_EXTERN NSString* const XCDMoviePlayerPlaybackDidFinishErrorUserInfoKey;

/**
 *  Posted when the video player has received the video information. The `object` of the notification is the
 * `XCDYouTubeVideoPlayerViewController` instance. The `userInfo` dictionary contains the `XCDYouTubeVideo` object.
 */
MP_EXTERN NSString* const XCDYouTubeVideoPlayerViewControllerDidReceiveVideoNotification;
/**
 *  The key for the `XCDYouTubeVideo` object in the user info dictionary of
 ...
 *
 *  @param videoIdentifier A 11 characters YouTube video identifier. If the video identifier is invalid the
 * `MPMoviePlayerPlaybackDidFinishNotification` will be posted with a `MPMovieFinishReasonPlaybackError` reason.
 *
 *  @return An initialized YouTube video player view controller with the specified video identifier.
 *
 *  @discussion You can pass a nil *videoIdentifier* (or use the standard `init` method instead) and set the
 * `<videoIdentifier>` property later.
 */
...
/**
 *  The preferred order for the quality of the video to play. Plays the first match when multiple video streams are
 * available.
 *
 *  Defaults to @[ XCDYouTubeVideoQualityHTTPLiveStreaming, @(XCDYouTubeVideoQualityHD720),
 * @(XCDYouTubeVideoQualityMedium360), @(XCDYouTubeVideoQualitySmall240) ]
 *
 *  You should set this property right after calling the `<initWithVideoIdentifier:>` method. Setting this property to
 * nil restores its default values.
 *
 ...
 *
 *  @discussion The video view is added as a subview of the specified view. The video does not start playing
 * immediately, you have to call `[videoPlayerViewController.moviePlayer play]` for playback to start. See
 * `MPMoviePlayerController` documentation for more information.
 *
...
MP_EXTERN NSString* const XCDYouTubeVideoPlayerViewControllerDidReceiveMetadataNotification
    DEPRECATED_MSG_ATTRIBUTE("Use XCDYouTubeVideoPlayerViewControllerDidReceiveVideoNotification instead.");
...
MP_EXTERN NSString* const XCDMetadataKeySmallThumbnailURL
    DEPRECATED_MSG_ATTRIBUTE("Use XCDYouTubeVideoUserInfoKey instead.");
