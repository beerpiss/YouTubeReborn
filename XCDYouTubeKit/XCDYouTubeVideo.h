 ...
 *
 *  The constant numbers are the YouTube
 * [itag](https://en.wikipedia.org/wiki/Talk:YouTube/Archive_22#Moved_from_YouTube#Quality_formats) values.
 */
...
/**
 *  Represents a YouTube video. Use the `<-[XCDYouTubeClient getVideoWithIdentifier:completionHandler:]>` method to
 * obtain a `XCDYouTubeVideo` object.
 */
 ...
 */
@property(nonatomic, readonly, nullable) NSURL* thumbnailURL DEPRECATED_MSG_ATTRIBUTE("Use `thumbnailURLs` instead.");
/**
 *  A thumbnail URL for an image of small size, i.e. 120×90. May be nil.
 */
@property(nonatomic, readonly, nullable)
    NSURL* smallThumbnailURL DEPRECATED_MSG_ATTRIBUTE("Renamed. Use `thumbnailURL` instead.");
/**
 *  A thumbnail URL for an image of medium size, i.e. 320×180, 480×360 or 640×480. May be nil.
 */
@property(nonatomic, readonly, nullable)
    NSURL* mediumThumbnailURL DEPRECATED_MSG_ATTRIBUTE("No longer available. Use `thumbnailURL` instead.");
/**
 *  A thumbnail URL for an image of large size, i.e. 1'280×720 or 1'980×1'080. May be nil.
 */
@property(nonatomic, readonly, nullable)
    NSURL* largeThumbnailURL DEPRECATED_MSG_ATTRIBUTE("No longer available. Use `thumbnailURL` instead.");

/**
 *  A dictionary of video stream URLs.
 *
 *  The keys are the YouTube [itag](https://en.wikipedia.org/wiki/YouTube#Quality_and_formats) values as `NSNumber`
 * objects. The values are the video URLs as `NSURL` objects. There is also the special
 * `XCDYouTubeVideoQualityHTTPLiveStreaming` key for live videos.
 *
...
*  The `streamURLs` may contain both video and audio streams, some video streams do not contain any audio. This property
will return a video stream that contains both audio and video with a maximum video quality of 720p in the case of videos
that aren't live. Also, this properly will return the URL to a live stream in the case of live videos.
*/
 ...
 *
 *  The keys are the [language codes](https://www.loc.gov/standards/iso639-2/php/code_list.php) values as `NSString`
 objects. The values are the caption URLs as `NSURL` objects.
 *
 ...
 *
 *  The keys are the [language codes](https://www.loc.gov/standards/iso639-2/php/code_list.php) values as `NSString`
 objects. The values are the caption URLs as `NSURL` objects. These URLs are the ones that were automatically generated
 by YouTube.
 *
 *  You should not store the URLs for later use since they have a limited lifetime.
 */
...
#endif
/**
 *  An array of strings (YouTube video identifier) of other streams, this indicates that the video is a multi-camera
 * video. Use  `<XCDYouTubeClient>  or  `<XCDYouTubeVideoOperation>  to fetch the other streams.
 */
#if __has_feature(objc_generics)
 ...
 *
 *  After this date, the stream URLs will not be playable. May be nil if it can not be determined, for example in live
 * videos.
 */
