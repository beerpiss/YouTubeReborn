...
NSString* const XCDMoviePlayerPlaybackDidFinishErrorUserInfoKey =
    @"error";  // documented in -[MPMoviePlayerController initWithContentURL:]

NSString* const XCDYouTubeVideoPlayerViewControllerDidReceiveMetadataNotification =
...
NSString* const XCDYouTubeVideoPlayerViewControllerDidReceiveVideoNotification =
...
- (instancetype)initWithContentURL:(NSURL*)contentURL {
    @throw [NSException exceptionWithName:NSGenericException
                                   reason:@"Use the `initWithVideoIdentifier:` method instead."
                                 userInfo:nil];
}  // LCOV_EXCL_LINE
...
- (instancetype)initWithVideoIdentifier:(NSString*)videoIdentifier {
    ...
    if (([callStackSymbols rangeOfString:@"-[XCDYouTubeClient getVideoWithIdentifier:completionHandler:]_block_invoke"]
             .length > 0) ||
        ([callStackSymbols
             rangeOfString:@"-[XCDYouTubeClient getVideoWithIdentifier:cookies:completionHandler:]_block_invoke"]
             .length > 0) ||
        ([callStackSymbols
             rangeOfString:
                 @"-[XCDYouTubeClient getVideoWithIdentifier:cookies:customPatterns:completionHandler:]_block_invoke"]
             .length > 0)) {
        NSString* reason = @"XCDYouTubeVideoPlayerViewController must not be used in the completion handler of "
                           @"`-[XCDYouTubeClient getVideoWithIdentifier:completionHandler:]` or `-[XCDYouTubeClient "
                           @"getVideoWithIdentifier:cookies:completionHandler:]` or `-[XCDYouTubeClient "
                           @"getVideoWithIdentifier:cookies:customPatterns:completionHandler:]`. Please read the "
                           @"documentation and sample code to properly use XCDYouTubeVideoPlayerViewController.";
...
- (NSArray*)preferredVideoQualities {
    if (!_preferredVideoQualities)
        _preferredVideoQualities = @[
            XCDYouTubeVideoQualityHTTPLiveStreaming, @(XCDYouTubeVideoQualityHD720), @(XCDYouTubeVideoQualityMedium360),
...
- (void)setVideoIdentifier:(NSString*)videoIdentifier {
    ...
    self.videoOperation = [[XCDYouTubeClient defaultClient]
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:XCDYouTubeVideoPlayerViewControllerDidReceiveMetadataNotification
                      object:self
                    userInfo:userInfo];
#pragma clang diagnostic pop
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationName:XCDYouTubeVideoPlayerViewControllerDidReceiveVideoNotification
                      object:self
