...
- (instancetype)init {
    @throw [NSException exceptionWithName:NSGenericException
                                   reason:@"Use the `initWithVideo:streamURLsToQuery:options:cookies:` method instead."
                                 userInfo:nil];
}
#pragma clang diagnostic pop

- (instancetype)initWithVideo:(XCDYouTubeVideo*)video
            streamURLsToQuery:(NSDictionary<id, NSURL*>*)streamURLsToQuery
                      options:(NSDictionary*)options
                      cookies:(NSArray<NSHTTPCookie*>*)cookies {
    if (video == nil)
        @throw [NSException exceptionWithName:NSInvalidArgumentException
                                       reason:@"`video` must not be nil"
                                     userInfo:nil];
    ...
    for (id key in streamURLsToQuery) {
        ...
        if (_video.streamURLs[key] == nil ||
            [(NSURL*)_video.streamURLs[key] isEqual:(NSURL*)streamURLsToQuery[key]] == NO) {
            continue;
        }
    ...
    _queryQueue.maxConcurrentOperationCount =
        6;  // paul_irish: Chrome re-confirmed that the 6 connections-per-host limit is the right magic number:
            // https://code.google.com/p/chromium/issues/detail?id=285567#c14
            // [https://twitter.com/paul_irish/status/422808635698212864]
...
- (void)startQuery {
    ...
    // Always use the `video` to check if it's a live stream (clients might not include
    // `XCDYouTubeVideoQualityHTTPLiveStreaming` in `streamURLsToQuery`)
    BOOL isHTTPLiveStream = self.video.streamURLs[XCDYouTubeVideoQualityHTTPLiveStreaming] != nil;

    [self.streamURLsToQuery
        enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, NSURL* _Nonnull obj, BOOL* _Nonnull stop) {
          XCDURLHEADOperation* operation = [[XCDURLHEADOperation alloc] initWithURL:obj
                                                                               info:@{key : obj}
                                                                             cookes:self.cookies];
    ...
    for (XCDURLHEADOperation* HEADOperation in HEADOperations) {
        ...
        if (HEADOperation.error == nil && [(NSHTTPURLResponse*)HEADOperation.response statusCode] == 200) {
            if (isHTTPLiveStream) {
                ...
                [GETOperations addObject:[[XCDURLGETOperation alloc] initWithURL:HEADOperation.url
                                                                            info:HEADOperation.info
                                                                          cookes:HEADOperation.cookies]];
            }
        }
    }
    ...
    if (isHTTPLiveStream) {
        /**
         * When it's a live stream all the other streams plus the live stream will cause the `XCDURLGetOperation` to
         * take a extremely longtime to complete. Since it's a live stream clients would tend to be only interested in
         * the `XCDYouTubeVideoQualityHTTPLiveStreaming` value and since we checked this already with the head operation
         * then we consider these URLs to be reachable
         */
...
- (void)startGETOperations:(NSArray<XCDURLGETOperation*>*)GETOperations
              streamErrors:(NSMutableDictionary<id, NSError*>*)streamErrors {
...
- (void)finishWithStreamURLs:(NSDictionary*)streamURLs streamErrors:(NSDictionary*)streamErrors {
    ...
    if (streamURLs.count == 0) {
        NSError* error = [NSError errorWithDomain:XCDYouTubeVideoErrorDomain
                                             code:XCDYouTubeErrorNoStreamAvailable
                                         userInfo:@{NSLocalizedDescriptionKey : @"No stream URLs are reachable."}];
