...
@property(atomic, strong)
    NSError* youTubeError;  // Error actually coming from the YouTube API, i.e. explicit and localized error
...
- (instancetype)init {
    @throw
        [NSException exceptionWithName:NSGenericException
                                reason:@"Use the `initWithVideoIdentifier:cookies:languageIdentifier:` method instead."
                              userInfo:nil];
}  // LCOV_EXCL_LINE
...
- (instancetype)initWithVideoIdentifier:(NSString*)videoIdentifier
                     languageIdentifier:(NSString*)languageIdentifier
                         ...
                         customPatterns:(NSArray<NSString*>*)customPatterns {
    if (!(self = [super init]))
...
- (instancetype)initWithVideoIdentifier:(NSString*)videoIdentifier languageIdentifier:(NSString*)languageIdentifier {
    return [self initWithVideoIdentifier:videoIdentifier
...
- (void)startNextRequest {
    if (self.eventLabels.count == 0) {
        ...
        NSString* urlString = [NSString
            stringWithFormat:@"https://www.youtube.com/youtubei/v1/player?key=%@", XCDYouTubeClient.innertubeApiKey];
        ...
        NSString* string = [NSString
            stringWithFormat:@"{'context': {'client': {'hl': 'en','clientName': 'ANDROID','clientVersion': "
                             @"'16.20','playbackContext': {'contentPlaybackContext': {'html5Preference': "
                             @"'HTML5_PREF_WANTS'}}}},'contentCheckOk': true,'racyCheckOk': true,'videoId': '%@'}",
                             self.videoIdentifier];
...
// Last resort request, sometimes the `javaScriptPlayerURL` will randomly not appear on `webpage` but will appear on the
// `embedWebpage` regardless of age restrictions. This appears to be how youtube-dl handles it, see
// https://github.com/l1ving/youtube-dl/blob/4fcd20a46cbf35ebbeaf06dde3999ad4309d7a8e/youtube_dl/extractor/youtube.py#L1963
...
- (void)startRequestWithURL:(NSURL*)url type:(XCDYouTubeRequestType)requestType {
    ...
    // Max (age-restricted VEVO) = 2×GetVideoInfo + 1×WatchPage + 2×EmbedPage + 1×JavaScriptPlayer + 1×GetVideoInfo +
    // 1xDashManifest
    if (++self.requestCount > 8) {
        // This condition should never happen but the request flow is quite complex so better abort here than go into an
    ...
    [request setValue:[NSString stringWithFormat:@"https://youtube.com/watch?v=%@", self.videoIdentifier]
    ...
    self.dataTask = [self.session dataTaskWithRequest:request
                                    completionHandler:^(NSData* data, NSURLResponse* response, NSError* error) {
                                      ...
                                      if (error)
                                          ...
                                          [self handleConnectionSuccessWithData:data
                                                                       response:response
...
- (void)handleConnectionSuccessWithData:(NSData*)data
                               response:(NSURLResponse*)response
                            requestType:(XCDYouTubeRequestType)requestType {
    CFStringEncoding encoding =
        CFStringConvertIANACharSetNameToEncoding((__bridge CFStringRef)response.textEncodingName ?: CFSTR(""));
    // Use kCFStringEncodingMacRoman as fallback because it defines characters for every byte value and is ASCII
    // compatible. See https://mikeash.com/pyblog/friday-qa-2010-02-19-character-encodings.html
    NSString* responseString =
        CFBridgingRelease(CFStringCreateWithBytes(
            kCFAllocatorDefault, data.bytes, (CFIndex)data.length,
            encoding != kCFStringEncodingInvalidId ? encoding : kCFStringEncodingMacRoman, false))
            ?: @"";
    ...
    if ([(NSHTTPURLResponse*)response statusCode] == 429) {
        ...
        [self handleConnectionError:
                  [NSError errorWithDomain:XCDYouTubeVideoErrorDomain
                                      code:XCDYouTubeErrorTooManyRequests
                                  userInfo:@{
                                      NSLocalizedDescriptionKey :
                                          @"The operation couldn’t be completed because too many requests were sent."
                                  }]
                        requestType:requestType];
        return;
    }
    if ([(NSHTTPURLResponse*)response statusCode] == 404 && responseString.length == 0) {
        [self handleConnectionError:[NSError errorWithDomain:XCDYouTubeVideoErrorDomain
                                                        code:XCDYouTubeErrorEmptyResponse
                                                    userInfo:@{NSLocalizedDescriptionKey : @"The response is empty."}]
                        requestType:requestType];
        return;
    }
    if (responseString.length == 0) {
        ...
        self.lastError = [NSError errorWithDomain:XCDYouTubeVideoErrorDomain
                                             code:XCDYouTubeErrorEmptyResponse
                                         userInfo:nil];
        XCDYouTubeLogError(@"Failed to decode response from %@ (response.textEncodingName = %@, data.length = %@)",
                           response.URL, response.textEncodingName, @(data.length));
        [self finishWithError];
        return;
    }
...
- (void)handleConnectionError:(NSError*)connectionError requestType:(XCDYouTubeRequestType)requestType {
    // Shoud not return a connection error if was as a result of requesting the Dash Manifiest (we have a sucessfully
    // created `XCDYouTubeVideo` and should just finish the operation as if were a 'sucessful' one
    ...
    NSDictionary* userInfo =
        @{NSLocalizedDescriptionKey : connectionError.localizedDescription, NSUnderlyingErrorKey : connectionError};
...
- (void)initializeConsentWithResponse:(NSURLResponse*)response {
    ...
    if (httpResponse && response.URL) {
        NSArray<NSHTTPCookie*>* cookies = [NSHTTPCookie cookiesWithResponseHeaderFields:httpResponse.allHeaderFields
                                                                                 forURL:(NSURL* _Nonnull)response.URL];

        for (NSHTTPCookie* cookie in cookies) {
            if ([cookie.name isEqualToString:@"__Secure-3PSID"])
                return;
        }

        for (NSHTTPCookie* cookie in cookies) {
            if ([cookie.name isEqualToString:@"CONSENT"]) {
                if ([cookie.value isEqualToString:@"YES"])
                    return;
                ...
                NSString* cookieValue = [[NSString alloc]
                    initWithFormat:@"YES+cb.%@-17-p0.en+FX+%i", [self youtubeConsentDateString], consentID];
...
- (void)handleWebPageWithHTMLString:(NSString*)html {
    ...
    if (self.webpage.javaScriptPlayerURL) {
        ...
        if (self.webpage.isAgeRestricted) {
            NSString* embedURLString =
                [NSString stringWithFormat:@"https://www.youtube.com/embed/%@", self.videoIdentifier];
            [self startRequestWithURL:[NSURL URLWithString:embedURLString] type:XCDYouTubeRequestTypeEmbedPage];
        } else {
            [self startNextRequest];
        }
...
- (void)handleJavaScriptPlayerWithScript:(NSString*)script {
    ...
    if (self.webpage.isAgeRestricted && self.cookies.count == 0) {
        ...
        NSString* queryString = XCDQueryStringWithDictionary(query);
        NSURL* videoInfoURL =
            [NSURL URLWithString:[@"https://www.youtube.com/get_video_info?" stringByAppendingString:queryString]];
...
- (void)finishWithVideo:(XCDYouTubeVideo*)video {
    ...
    XCDYouTubeLogDebug(@"%@", ^{
      return video.debugDescription;
    }());
    [self finish];
}

- (void)finishWithError {
    self.error = self.youTubeError
                     ? YouTubeError(self.youTubeError, self.webpage.regionsAllowed, self.languageIdentifier)
                     : self.lastError;
    if (self.error == nil) {
        // This condition should never happen but as a last resort.
        // See https://github.com/0xced/XCDYouTubeKit/issues/484
        self.error = [NSError
            errorWithDomain:XCDYouTubeVideoErrorDomain
                       code:XCDYouTubeErrorUnknown
                   userInfo:@{
                       NSLocalizedDescriptionKey : @"The operation couldn’t be completed because of an unknown error."
                   }];
    }
    XCDYouTubeLogError(@"Video operation finished with error: %@\nDomain: %@\nCode:   %@\nUser Info: %@",
                       self.error.localizedDescription, self.error.domain, @(self.error.code), self.error.userInfo);
    [self finish];
}
...
+ (BOOL)automaticallyNotifiesObserversForKey:(NSString*)key {
    ...
    return selector == @selector(isExecuting) || selector == @selector(isFinished) ||
...
- (void)cancel {
    ...
    // Wait for `start` to be called in order to avoid this warning: *** XCDYouTubeVideoOperation 0x7f8b18c84880 went
    // isFinished=YES without being started by the queue it is in
    dispatch_semaphore_wait(self.operationStartSemaphore,
...
- (NSString*)description {
    return [NSString
        stringWithFormat:@"<%@: %p> %@ (%@)", self.class, self, self.videoIdentifier, self.languageIdentifier];
}
