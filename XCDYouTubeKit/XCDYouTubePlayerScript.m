...
- (instancetype)initWithString:(NSString*)string customPatterns:(NSArray<NSString*>*)customPatterns {
    ...
    NSRegularExpression* anonymousFunctionRegularExpression =
        [NSRegularExpression regularExpressionWithPattern:@"\\(function\\(([^)]*)\\)\\{(.*)\\}\\)\\(([^)]*)\\)"
                                                  options:NSRegularExpressionDotMatchesLineSeparators
                                                    error:NULL];
    NSTextCheckingResult* anonymousFunctionResult =
        [anonymousFunctionRegularExpression firstMatchInString:script
                                                       options:(NSMatchingOptions)0
                                                         range:NSMakeRange(0, script.length)];
    if (anonymousFunctionResult.numberOfRanges > 3) {
        NSArray* parameters =
            [[script substringWithRange:[anonymousFunctionResult rangeAtIndex:1]] componentsSeparatedByString:@","];
        NSArray* arguments =
            [[script substringWithRange:[anonymousFunctionResult rangeAtIndex:3]] componentsSeparatedByString:@","];
    ...
    // See list of regex patterns here
    ...
    NSArray<NSString*>* hardCodedPatterns = @[
        ...
        /*The rest patterns are supposed to be obsolete but I am keep them here in case some older pattern matches the
         *YouTube API in the future IMPORTANT: Please note that the patterns above should be placed in the same order as
         *seen in youtube-dl here:
         *https://github.com/ytdl-org/youtube-dl/blob/master/youtube_dl/extractor/youtube.py#L1344 If they do not match
         *the same order some video won't play because of using the wrong signature.
         */
    ...
    for (NSRegularExpression* regularExpression in customPatternsExpressions.count == 0 ? hardCodedPatternsExpressions
                                                                                        : customPatternsExpressions) {
        if (_signatureFunction)
            break;

        NSArray<NSTextCheckingResult*>* regexResults =
            [regularExpression matchesInString:script options:(NSMatchingOptions)0 range:NSMakeRange(0, script.length)];

        for (NSTextCheckingResult* signatureResult in regexResults) {
            NSString* signatureFunctionName =
                signatureResult.numberOfRanges > 1 ? [script substringWithRange:[signatureResult rangeAtIndex:1]] : nil;
