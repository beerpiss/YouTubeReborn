...
typedef NS_ENUM(NSUInteger, XCDLogLevel) {
    ...
    /**
     *  Used when logging normal operational information, e.g. when a `<XCDYouTubeVideoOperation>` starts, is cancelled
     * or finishes.
     */
    XCDLogLevelInfo = 2,
 ...
 *
 *  By default, logs are emitted through CocoaLumberjack if it is available, i.e. if the `DDLog` class is found at
 * runtime. The [context][1] used for CocoaLumberjack is the `XCDYouTubeKitLumberjackContext` constant whose value is
 ...
 *
 *  If you are not using CocoaLumberjack, you can control the log levels with the `XCDYouTubeKitLogLevel` environment
 * variable. See also the `<XCDLogLevel>` enum.
 *
 *  Level   | Value | Mask
 *  --------|-------|------
 *  Error   |   0   | 0x01
 ...
 *  Verbose |   4   | 0x10
 *
 *  Use the corresponding bitmask to combine levels. For example, if you want to log *error*, *warning* and *info*
 * levels, set the `XCDYouTubeKitLogLevel` environment variable to `0x7` (0x01 | 0x02 | 0x04).
 *
...
/**
 *  If you prefer not to use CocoaLumberjack and want something more advanced than the default `NSLog` implementation,
 * you can use this method to write your own logger.
 *
 *  @param logHandler The block called when a log is emitted by the XCDYouTubeKit framework. If you set the log handler
 * to nil, logging will be completely disabled.
 *
 ...
 *  - The `function` parameter is the function name, captured with the `__PRETTY_FUNCTION__` macro where the log is
 * emitted.
 ...
 *
 *  ```
 *  [XCDYouTubeLogger setLogHandler:^(NSString * (^message)(void), XCDLogLevel level, const char *file, const char
 * *function, NSUInteger line) { LogMessageRawF(file, (int)line, function, @"XCDYouTubeKit", (int)level, message());
 *  }];
 *  ```
 */
+ (void)setLogHandler:
    (void (^)(NSString* (^message)(void), XCDLogLevel level, const char* file, const char* function, NSUInteger line))
        logHandler;
