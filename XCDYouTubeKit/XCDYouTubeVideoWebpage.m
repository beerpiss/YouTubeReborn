...
- (NSDictionary*)playerConfiguration {
    if (!_playerConfiguration) {
        NSArray<NSString*>* patterns =
            @[ @";ytplayer\\.config\\s*=\\s*(\\{.+?\\});ytplayer", @";ytplayer\\.config\\s*=\\s*(\\{.+?\\});" ];
...
static NSDictionary* XCDPlayerConfigurationWithString(NSString* html, NSString* regularExpressionPattern) {
    ...
    NSRegularExpression* playerConfigRegularExpression =
        [NSRegularExpression regularExpressionWithPattern:regularExpressionPattern
                                                  options:NSRegularExpressionCaseInsensitive
                                                    error:NULL];
    [playerConfigRegularExpression
        enumerateMatchesInString:html
                      ...
                      usingBlock:^(NSTextCheckingResult* result, NSMatchingFlags flags, BOOL* stop) {
                        for (NSUInteger i = 1; i < result.numberOfRanges; i++) {
                            ...
                            NSDictionary* playerConfiguration =
                                [NSJSONSerialization JSONObjectWithData:configData ?: [NSData new]
...
- (NSDictionary*)videoInfo {
    if (!_videoInfo) {
        ...
        if (args == nil) {
            _videoInfo =
                XCDPlayerConfigurationWithString(self.html, @"ytInitialPlayerResponse\\s*=\\s*(\\{.+?\\})\\s*;");
            return _videoInfo;
        }
        if ([args isKindOfClass:[NSDictionary class]]) {
            ...
            [args enumerateKeysAndObjectsUsingBlock:^(id key, id value, BOOL* stop) {
              if ([(NSObject*)value isKindOfClass:[NSString class]] ||
                  [(NSObject*)value isKindOfClass:[NSNumber class]])
...
- (NSString*)sts {
    if (!_sts) {
        ...
        if (sts != nil) {
            ...
            NSRegularExpression* regex = [NSRegularExpression regularExpressionWithPattern:@"\"sts\"\\s*:\\s*(\\d+)"
                                                                                   options:0
                                                                                     error:nil];
            NSTextCheckingResult* result = [regex firstMatchInString:self.html
                                                             options:(NSMatchingOptions)0
                                                               range:NSMakeRange(0, self.html.length)];
...
- (NSURL*)javaScriptPlayerURL {
    if (!_javaScriptPlayerURL) {
        ...
        if ([jsAssets isKindOfClass:[NSString class]]) {
            ...
            NSArray<NSString*>* patterns = @[
                @"<script[^>]+\\bsrc=(\"[^\"]+\")[^>]+\\bname=[\"\\\']player_ias/base", @"jsUrl\"\\s*:\\s*(\"[^\"]+\")",
                @"\"assets\":.+?\"js\":\\s*(\"[^\"]+\")"
            ];
            for (NSString* pattern in patterns) {
                ...
                NSString* baseURLString =
                    [[[self.html substringWithRange:range] stringByReplacingOccurrencesOfString:@"\"" withString:@""]
...
- (BOOL)isAgeRestricted {
    if (!_isAgeRestricted) {
        ...
        _isAgeRestricted =
            [self.html rangeOfString:@"og:restrictions:age" options:options range:range].location != NSNotFound ||
            [self.html rangeOfString:@"player-age-gate-content" options:options range:range].location != NSNotFound;
    }
...
- (NSSet*)regionsAllowed {
    if (!_regionsAllowed) {
        ...
        NSRegularExpression* regionsAllowedRegularExpression =
            [NSRegularExpression regularExpressionWithPattern:@"meta\\s+itemprop=\"regionsAllowed\"\\s+content=\"(.*)\""
                                                      options:(NSRegularExpressionOptions)0
                                                        error:NULL];
        NSTextCheckingResult* regionsAllowedResult =
            [regionsAllowedRegularExpression firstMatchInString:self.html
                                                        options:(NSMatchingOptions)0
                                                          range:NSMakeRange(0, self.html.length)];
