...
- (void)finishWithError:(NSError*)error {
    ...
    if (error.code == NSURLErrorNetworkConnectionLost && [error.domain isEqual:NSURLErrorDomain]) {
        // This might have failed because the file on YouTube's servers is incomplete. See
        // https://github.com/0xced/XCDYouTubeKit/issues/456 for more info.
        ...
        self.error = [NSError errorWithDomain:(NSString*)self.error.domain
                                         code:self.error.code
                                     userInfo:modifiedUserInfo.copy];
    }
...
- (void)startRequest {
    ...
    [request addValue:[NSString stringWithFormat:@"bytes=%@-%@", @(rangeStart), @(rangeEnd)]
