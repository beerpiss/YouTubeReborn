...
/**
 *  A key that may be present in the error's userInfo dictionary when the error code is
 * XCDYouTubeErrorNoStreamAvailable. The object for that key is a NSSet instance containing localized country names.
 */
extern NSString* const XCDYouTubeAllowedCountriesUserInfoKey;

/**
 *  These values are returned as the error code property of an NSError object with the domain
...
typedef NS_ENUM(NSInteger, XCDYouTubeErrorCode) {
    ...
    /**
     *  Returned when a network error occurs. See `NSUnderlyingErrorKey` in the userInfo dictionary for more
     * information.
     */
    XCDYouTubeErrorNetwork = -1,
    /**
     * Returned when an empty response is returned. This may indicate that YouTube has blocked requests from your IP
     * address because of overuse. This error does not contain any info in the `userInfo` property and is not
     * appropriate to show to the user.
     */
    XCDYouTubeErrorEmptyResponse = -3,
    /**
     * Returned when an 429 HTTP code is returned. This may indicate that YouTube has blocked requests from your IP
     * address because of overuse. This error code will the be code of the `NSError` in `NSUnderlyingErrorKey` when
     * `XCDYouTubeErrorNetwork` is return when appropriate. This error is not localized.
     */
    ...
    /**
     *  Previously returned when the video was removed as a violation of YouTube's policy or when the video did not
     * exist.
     */
