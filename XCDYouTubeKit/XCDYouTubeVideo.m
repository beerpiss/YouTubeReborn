...
NSArray<NSDictionary*>* XCDThumnailArrayWithString(NSString* string) {
    ...
    if (thumbnails.count == 0 || thumbnails == nil) {
        return nil;
    }
    return thumbnails;
}
...
NSArray<NSDictionary*>* XCDCaptionArrayWithString(NSString* string) {
    ...
    if (captionTracks.count == 0 || captionTracks == nil) {
        return nil;
    }
    ...
    for (NSString* field in fields) {
        ...
        if (pair.count == 2) {
            ...
            if (dictionary[key] && ![(NSObject*)dictionary[key] isEqual:value]) {
                XCDYouTubeLogWarning(@"Using XCDDictionaryWithQueryString is inappropriate because the query string "
                                     @"has multiple values for the key '%@'\n"
                                     @"Query: %@\n"
                                     @"Discarded value: %@",
...
static NSURL* URLBySettingParameter(NSURL* URL, NSString* key, NSString* percentEncodedValue) {
    ...
    NSRegularExpression* regularExpression =
        [NSRegularExpression regularExpressionWithPattern:pattern options:(NSRegularExpressionOptions)0 error:NULL];
    NSMutableString* percentEncodedQuery = [components.percentEncodedQuery ?: @"" mutableCopy];
    NSUInteger numberOfMatches = [regularExpression replaceMatchesInString:percentEncodedQuery
                                                                   options:(NSMatchingOptions)0
                                                                     range:NSMakeRange(0, percentEncodedQuery.length)
                                                              withTemplate:template];
    if (numberOfMatches == 0)
        [percentEncodedQuery
            appendFormat:@"%@%@=%@", percentEncodedQuery.length > 0 ? @"&" : @"", key, percentEncodedValue];
...
- (instancetype)initWithIdentifier:(NSString*)identifier
    ...
    NSArray* alternativeStreamMap = XCDStreamingDataWithString(playerResponse)[@"formats"] == nil
                                        ? info[@"streamingData"][@"formats"]
                                        : XCDStreamingDataWithString(playerResponse)[@"formats"];
    ...
    NSArray* alternativeAdaptiveFormats = XCDStreamingDataWithString(playerResponse)[@"adaptiveFormats"] == nil
                                              ? info[@"streamingData"][@"adaptiveFormats"]
                                              : XCDStreamingDataWithString(playerResponse)[@"adaptiveFormats"];
    NSDictionary* videoDetails = XCDDictionaryWithString(playerResponse)[@"videoDetails"] == nil
                                     ? info[@"videoDetails"]
                                     : XCDDictionaryWithString(playerResponse)[@"videoDetails"];
    NSString* multiCameraMetadataMap =
        XCDDictionaryWithString(playerResponse)[@"multicamera"][@"playerLegacyMulticameraRenderer"][@"metadataList"];

    NSMutableDictionary* userInfo =
        response.URL ? [@{NSURLErrorKey : (id)response.URL} mutableCopy] : [NSMutableDictionary new];

    if (streamMap.length > 0 || httpLiveStream.length > 0 || alternativeStreamMap.count > 0 ||
        alternativeAdaptiveFormats.count > 0) {
        ...
        _viewCount = info[@"viewCount"] == nil ? [(NSString*)videoDetails[@"viewCount"] integerValue]
                                               : [(NSString*)info[@"viewCount"] integerValue];

        _duration = info[@"length_seconds"] == nil ? [(NSString*)videoDetails[@"lengthSeconds"] doubleValue]
                                                   : [(NSString*)info[@"length_seconds"] doubleValue];
        ...
        if (!_thumbnailURL) {
            ...
            if (thumbnails.count >= 1) {
                ...
                NSMutableArray<NSURL*>* thumbnailURLs =
                    [[NSMutableArray<NSURL*> alloc] initWithCapacity:thumbnails.count];
        ...
        for (NSDictionary* caption in XCDCaptionArrayWithString(playerResponse)) {
...

            {
                if ([languageCode isEqualToString:@"und"]) {
                    // Skip because this is a special code than is used to indicate that the lanauage code is
                    // undetermined.
                    continue;
                }
        ...
        NSDictionary* mainStreamURLs =
            [self extractStreamURLsWithQuery:streamQueries.count == 0 ? alternativeStreamQueries : streamQueries
        ...
        if (_streamURLs.count == 0) {
            if (error)
                *error = [NSError errorWithDomain:XCDYouTubeVideoErrorDomain
        ...
        if (error) {
            ...
            if (reason) {
                reason = [reason stringByReplacingOccurrencesOfString:@"<br\\s*/?>"
                                                           withString:@" "
                ...
                reason = [reason stringByReplacingOccurrencesOfString:@"\n"
                                                           withString:@" "
                                                              options:(NSStringCompareOptions)0
                                                                range:NSMakeRange(0, reason.length)];
                NSRange range;
                while ((range = [reason rangeOfString:@"<[^>]+>" options:NSRegularExpressionSearch]).location !=
                       NSNotFound)
...
static NSString* XCDReasonForErrorWithDictionary(NSDictionary* info, NSString* playerResponse) {
    NSString* reason = info[@"reason"] == nil ? XCDDictionaryWithString(playerResponse)[@"playabilityStatus"][@"reason"]
                                              : info[@"reason"];
    NSDictionary* subReason = XCDDictionaryWithString(
        playerResponse)[@"playabilityStatus"][@"errorScreen"][@"playerErrorMessageRenderer"][@"subreason"];
...
- (NSDictionary<id, NSURL*>*)extractStreamURLsWithQuery:(NSArray*)streamQueries
                                           playerScript:(XCDYouTubePlayerScript*)playerScript
    ...
    for (NSObject* streamQuery in streamQueries) {
        ...
        NSDictionary* alternativeStreamInfo = XCDDictionaryWithQueryString(stream[@"cipher"]).count == 0
                                                  ? XCDDictionaryWithQueryString(stream[@"signatureCipher"])
                                                  : XCDDictionaryWithQueryString(stream[@"cipher"]);
        ...
        if (scrambledSignature && !playerScript) {
            ...
            if (error)
                *error = [NSError errorWithDomain:XCDYouTubeVideoErrorDomain
                                             code:XCDYouTubeErrorUseCipherSignature
                                         userInfo:userInfo];
...
- (BOOL)isEqual:(id)object {
    return [(NSObject*)object isKindOfClass:[XCDYouTubeVideo class]] &&
           [((XCDYouTubeVideo*)object).identifier isEqual:self.identifier];
}
...
- (NSString*)debugDescription {
    ...
    NSString* duration = [dateComponentsFormatter stringFromTimeInterval:self.duration]
                             ?: [NSString stringWithFormat:@"%@ seconds", @(self.duration)];
    ...
    return [NSString stringWithFormat:@"<%@: %p> %@\nDuration: %@\nExpiration date: %@\n%@\nStreams: %@", self.class,
                                      self, self.description, duration, self.expirationDate, thumbnailDescription,
                                      streamsDescription];
}
